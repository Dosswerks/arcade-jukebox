...
async function initApp() {
  ...
  // tracks.js may define window.PLAYLISTS (playlist-aware format) instead of window.TRACKS.
  // Resolve the active playlist into the flat TRACKS array the rest of the app expects.
  if (typeof window.TRACKS === 'undefined' && typeof window.PLAYLISTS !== 'undefined') {
    const params = new URLSearchParams(window.location.search);
    const playlistKey = params.get('playlist') || 'default';
    const playlist = window.PLAYLISTS[playlistKey] || window.PLAYLISTS['default'];
    window.TRACKS = playlist ? playlist.tracks : [];
  }
  ...
  // Media Session API — enables lock-screen playback on iOS/Android
  function setupMediaSession() {
    ...
    navigator.mediaSession.setActionHandler('seekto', (details) => {
      if (details.seekTime != null) {
        const duration = playerEngine.getProgress().duration;
        if (duration > 0) {
          playerEngine.seekTo(details.seekTime / duration);
        }
      }
    });
  }
  ...
  // Update media session on player events
  playerEngine.on('trackChange', (data) => {
    updateMediaSessionMetadata(data.track);
  });
